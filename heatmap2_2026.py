"""Second heatmap (v2): 1000-cell grid for 2026.
Each whole run = one colored block. New color scheme (slow -> fast):
  light green > 7:00, green 6:30-7:00, yellow 6:00-6:30, orange 5:30-6:00,
  red 5:00-5:30, blue 4:30-5:00, purple 4:15-4:30, pink < 4:15.
Run boundaries are marked with a thick black border on the last cell of each run.
"""
import json
import sys
...
OUT_XLSX = r"C:\Users\mares\Desktop\garmin\plan_2026_behy_v2.xlsx"
YEAR = 2026
GOAL_KM = 1000
COLS = 50; ROWS = 20

# Slowest -> fastest, with seconds/km lower & upper bounds.
PACE_TIERS = [
...
def main():
    ...
    print(f"2026 runs: {len(valid)}; total km {cum_km:.2f} -> {colored_n} cells")

    # Flatten per-cell paint info + per-run last-cell flag
    cell_paint = []   # (color_hex, tooltip, is_last_of_run)
    ...
    ws = wb.active
    ws.title = "Plán 2026 — běhy"
    thin = Side(border_style="thin", color="DDDDDD")
    thick_black = Side(border_style="thick", color="000000")
    center = Alignment(horizontal="center", vertical="center")

    ws.cell(row=1, column=1,
            value=f"Plán 2026 — 1000 km (běh = blok, tlustá čára = konec běhu)")
    ...
    # Helper: build border for a given cell at (i)
    # Default: thin gray everywhere. If end-of-run AND not last colored cell:
    #   - if col < COLS: thick right
    #   - if col == COLS: thick bottom (wraps visually)
    for i in range(GOAL_KM):
        ...
        if i < len(cell_paint):
            ...
            # mark end-of-run boundary (skip the very last colored cell of all)
    ...
    # Re-apply: if a cell's LEFT neighbor had a thick right boundary, mirror it on its left
    # (Excel renders shared borders; this prevents the line from being painted over).
    for i in range(1, GOAL_KM):
        if i - 1 < len(cell_paint) and cell_paint[i - 1][2] and i - 1 != len(cell_paint) - 1:
            r_prev = GRID_TOP + ((i - 1) // COLS)
            c_prev = 1 + ((i - 1) % COLS)
            ...
            b = cell_cur.border
            if c_prev == COLS:
                # boundary was on bottom of prev (col 50). The cell visually "below" prev
                # at column 1 of next row gets a thick top.
